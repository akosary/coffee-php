...
//$indexes[0]['product_id']
    ...
    {
        $this->query = "SELECT user.Id , user.name , sum(orders.total_price) AS 'Total' FROM `user` INNER JOIN `orders` WHERE user.Id = orders.id_user AND orders.created_at >= '$from' AND orders.created_at <= '$to' GROUP BY user.name";
        // $this->query = "SELECT id_user FROM `orders` WHERE orders.created_at >= '$from' AND orders.created_at <= '$to' ";
        $this->sql= $this->con->prepare($this->query);
        $this->sql->execute();
        $index = $this->sql->fetchAll(PDO::FETCH_ASSOC);
        if($index)
